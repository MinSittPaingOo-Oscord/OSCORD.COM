     ...
     <b>Course Fee</b> : " . $row2['courseFee'] . "<br><b>Course Period</b> : " . $row2['coursePeriod'] ."
    	...
    	<div class='tab'>
        	<ul class="nav nav-tabs" id="myTab" role="tablist">
            		...
            		<li class="nav-item" role="presentation">
                			<button class="nav-link" id="studentReview" data-bs-toggle="tab" data-bs-target="#student_review" type="button" role="tab" aria-controls="student_review" aria-selected="false">Student Review</button>
            		</li>   
                        ...
                        if ($result4 && $result4->num_rows > 0) {
   								 while ($row4 = $result4->fetch_assoc()) {
        								...
        								<div class='video-row row'>
            								<div class='col-md-6'>
    <iframe width='100%' height='auto' src='".$row4['videoLink']."' title='".$row4['videoName']."' frameborder='0' allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share' referrerpolicy='strict-origin-when-cross-origin' allowfullscreen></iframe>
            								</div>
            								<div class='col-md-6 go_here'>
                								<h2>".$row4['videoName']."</h2>
                								<p>In case the video frame doesn't work correctly : <a href='".$row4['videoLink']."'>go here</a></p>
            								</div>
        								</div>
        								<hr>
        							";
            ...
            <div class="tab-pane fade under" id="student_review" role="tabpanel" aria-labelledby="studentReview" tabindex="0">
              
                    <?php
                    $query5 = "SELECT * FROM oscord_studentreview WHERE courseID=".$id; 
                     $result5 = $conn->query($query5);
                    
                    while($row5 = $result5->fetch_assoc()){
                            
                            
                            $query6 = "SELECT * FROM oscord_student WHERE studentID =".$row5['studentID'];
                            $result6 = $conn->query($query6);
                            
                            while($row6 = $result6->fetch_assoc()){
                                    
                                    echo "<h1 id='student_name'> Student Name : ".$row6['studentName']."</h1>";
                            }
                    ...
                    <h2 class="text-center mb-4">review this course</h2>
					<form class='form' action='oscord_savereview.php' method='post'>
						<div class="mb-3">
                    		<label for="role" class="form-label">Select your name</label>
                    			<select class="form-select" id="role" name="student_name" required>
                        			<option value="">Select</option>
                        			
                                        
                                     <?php
                                        $query7 = "";
                            				$query7 = "SELECT s.* FROM oscord_student s JOIN oscord_studentxcourse sc ON s.studentID = sc.studentID WHERE sc.courseID =". $id;
                                        $result7 = $conn->query($query7);
                                        
                                        
                                        	while($row7 = $result7->fetch_assoc()){
                                               		 
                                                echo "<option value='".$row7['studentID']."'>".$row7['studentName']."</option>";
                                         	}
                         ...
                         <div class="mb-3">
        					...
        					<textarea class="form-control" id="textarea_review" rows="5" placeholder="type here" name="review_text"></textarea>
        					...
        					if ($result->num_rows > 0) {
            						echo "<div class='card shadow p-4 fileBox'>";
            							...
            							while ($row = $result->fetch_assoc()) {
                								...
                								echo "<a href='download.php?id=" . $row['fileID'] . "' class='text-decoration-none text-dark'>" . $row['fileName'] . "</a>";
                       ...
                       <?php 
                        ...
                        if ($result9 && $result9->num_rows > 0) {
   								 while ($row9 = $result9->fetch_assoc()) {
        								...
        								<div class='video-row row'>
            								<div class='col-md-6'>
    <iframe width='100%' height='auto' src='".$row9['videoLink']."' title='".$row9['videoName']."' frameborder='0' allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share' referrerpolicy='strict-origin-when-cross-origin' allowfullscreen></iframe>
            								</div>
            								<div class='col-md-6 go_here'>
                								<h2>".$row9['videoName']."</h2>
                								<p>In case the video frame doesn't work correctly : <a href='".$row9['videoLink']."'>go here</a></p>
            								</div>
        								</div>
        								<hr>
        							";
